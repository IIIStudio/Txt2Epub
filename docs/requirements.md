...
| F7-6 | **编辑区即所见即所得**（原「实时预览窗」已移除）。<br>原方案：右栏独立预览面板 + 「适应/手机/平板」宽度切换。问题——三个宽度差别极小形同虚设；预览与编辑区是两套样式，改了必须切过去看，且常因特异性不同而结果不一致；还多占一栏。<br>现方案：把**导出 CSS 原样作用域化到 `.editor-surface`** 注入编辑区，编辑区所见即导出所得。改排版/高亮规则/自定义 CSS 后编辑区立即反映，无需切换面板。<br>配套：章节标题是 `<input>`（不支持伪元素），故用 `.chapter-title-rule` 元素模拟 `h1.chapter-title::before` 的几何短线，并把标题字号/字重/颜色/对齐同步给输入框；「纸张」容器用 `--doc-bg`/`--doc-fg` 变量跟随文档主题 | P0 |
...
| F7-20 | **默认排版**（中文电子书常规 + 国际风克制配色）：<br>· **字体默认「系统默认」**（零依赖、加载最快、兼容性最好），并且它是 chips 的**第一项**——默认值与选项顺序必须保持一致<br>· **首行缩进两字符 + 标题居中**：默认开启（中文电子书标准做法）。开启缩进时自动把段间距缩到 45%，避免与缩进重复分隔段落<br>· 正文**左对齐 ragged right**（中文两端对齐会产生难看的字间距，可开关）<br>· 标题负字距（-0.011em ~ -0.02em）、上疏下密（亲密性原则）<br>· 章节标题用**几何短线**装饰（`::before`，2.2em × 2px），非花哨纹样<br>· 强调用字重而非颜色<br>· 克制中性色：正文 `#18181B`（近黑非纯黑）、次要 `#71717A`、分隔线 `#E4E4E7`<br>· 行高 1.75、页边距 7%<br>所有项均可在「排版」面板调整（无缩进 / 标题左对齐 / 两端对齐 / 衬线字体） | P0 |
| F7-23 | 默认值变更时，既有草稿（IndexedDB）中的旧值会**覆盖**新默认值（`Object.assign({}, DEFAULT_TYPO, p.typography)`）。<br>因此改默认值后，老用户需点「清空」或手动切换才能看到新默认；反馈默认值问题时必须先确认是默认值本身还是草稿残留 | P0 |
| F7-22 | **选项顺序须跟随默认值**：默认值变更时，「正文字体」等 chips 的顺序必须同步调整，让默认项排在第一位。<br>（历史教训：默认值从 serif → sans → system 改了两次，chips 顺序未同步，导致「系统默认」是默认值却排在最后） | P1 |
...
| F9-3 | **工程 JSON 双向**：「保存工程」弹窗提供 `导出 JSON` / `导入 JSON` / `保存到本机`。<br>往返无损：章节、正文、图片资源、元数据、排版设置、分章设置、高亮规则、自定义 CSS 全部还原 | P1 |
| F9-3a | 导入前须二次确认（列出书名/章节数/图片数，明确提示「覆盖当前工程」），并提供「取消」出口 | P1 |
| F9-3b | **草稿恢复与 JSON 导入必须共用同一套规范化**（`normalizeProject`）。若各写一套补齐逻辑，会出现「导入后某些设置丢失」 | P1 |
| F9-3c | 向后兼容：旧工程 / 手改过的 JSON 缺失字段时用默认值补齐（章节 id、`enabled`、排版、规则、`assets`），并过滤无 `dataUrl` 的坏资源 | P1 |
| F9-3d | 错误处理：非 JSON、JSON 数组、缺 `chapters` 字段须给出可读提示，不得抛异常 | P1 |
| F9-3e | 支持拖入 `.json` 文件直接导入；导入前须 `cancel()` 待执行的 debounce 自动保存，避免旧内容被写回草稿 | P2 |
